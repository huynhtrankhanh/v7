// Stress test: compare reverse_algorithm against original parse/assemble logic
// Run with: node stress_test.js

const { syllableToStroke, strokeToVietnamese } = require("./reverse_algorithm");

// Original mappings (copied from static/script.js)
...
// --- Original parse/assemble (copy) ---
function parseOriginal(stroke) {
...
function assembleOriginal(parsed) {
    const initial = () => {
...
function main() {
                ...
                for (const vowel of vowelOptions) {
                    for (const final of finalOptions) {
                        for (const tone of toneOptions) {
                            ...
                            const parsed = parseOriginal(stroke);
                            if (!parsed) continue;
                            const syllable = assembleOriginal(parsed);
                            const backStroke = syllableToStroke(syllable);
                            if (!backStroke) {
                                // Skip unattested/unsupported syllables
                                continue;
                            }
                            const fwd = strokeToVietnamese(backStroke);
                            const parsedBack = parseOriginal(backStroke);
                            const syllableBack = parsedBack ? assembleOriginal(parsedBack) : null;
                            if (fwd !== syllable || syllableBack !== syllable) {
                                throw new Error(`Mismatch ${syllable}: backStroke=${backStroke}, fwd=${fwd}, orig=${syllableBack}`);
                            }
                            checked++;
                        }
                    }
    ...
    console.log(`Stress test passed: ${checked} syllables`);
}
...
module.exports = { main, parseOriginal, assembleOriginal };
