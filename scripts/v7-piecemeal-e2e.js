...
function waitForOutput(proc, substring, timeoutMs = 30000) {
  ...
  return new Promise((resolve, reject) => {
    const timer = setTimeout(
      () =>
        reject(
          new Error(`Timed out waiting for "${substring}". Logs:\n${logs}`),
        ),
      timeoutMs,
    );
    const onData = (data) => {
...
function parseCode(code) {
  const consonants = [...LEFT_CONSONANTS.keys()].sort(
...
function leftKeys(code) {
  ...
  if (mask === undefined)
    throw new Error(`Unsupported left consonant: ${parsed.consonant}`);
  const keys = [];
  ...
  if (!["a", "o", "i"].includes(parsed.vowel))
    throw new Error(`E2E avoids suffix vowels, got: ${code}`);
...
function rightKeys(code) {
  ...
  if (mask === undefined)
    throw new Error(`Unsupported right consonant: ${parsed.consonant}`);
  const keys = [];
...
async function waitForMarkerCount(page, count) {
  await page.waitForFunction(
    (expected) =>
      document.querySelectorAll(".piecemeal-syllable").length === expected,
    { timeout: 10000 },
    count,
  );
}
...
async function assertMarkerTexts(page, expected, label) {
  await page.waitForFunction(
    (count) =>
      document.querySelectorAll(".piecemeal-syllable").length === count,
    { timeout: 10000 },
    expected.length,
  );
  ...
  if (JSON.stringify(texts) !== JSON.stringify(expected)) {
    throw new Error(
      `${label} marker text mismatch. Expected ${JSON.stringify(expected)}, got ${JSON.stringify(texts)}`,
    );
  }
}
...
async function main() {
  ...
  try {
    ...
    for (let i = 0; i < fixedChords.length; i++) {
      ...
      await assertMarkerTexts(
        page,
        fixedChords.slice(0, i + 1).map((entry) => entry.text),
        `fixed-only ${i + 1}`,
      );
    }
    ...
    const fixedMarkerTexts = initialMarkers
      .slice(0, fixedChords.length)
      .map((marker) => marker.text);
    const expectedFixedTexts = fixedChords.map((entry) => entry.text);
    if (
      JSON.stringify(fixedMarkerTexts) !== JSON.stringify(expectedFixedTexts)
    ) {
      throw new Error(
        `Fixed syllables were not preserved in mixed markers. Expected ${JSON.stringify(expectedFixedTexts)}, got ${JSON.stringify(fixedMarkerTexts)}`,
      );
    }
    if (initialMarkers.some((marker) => marker.text.trim() === "")) {
      throw new Error(
        `Blank marker after V7 entry: ${JSON.stringify(initialMarkers)}`,
      );
    }
    ...
    await page.waitForFunction(
      () => {
        const markers = Array.from(
          document.querySelectorAll(".piecemeal-syllable"),
        );
    ...
    for (let domIndex = 1; domIndex < 9; domIndex++) {
      ...
      await page.waitForFunction(
        (expectedDomIndex) => {
          ...
          return (
            markers.length === 9 &&
            markers[expectedDomIndex]?.classList.contains("active")
          );
        },
        { timeout: 5000 },
        domIndex,
      ...
      if (activeCount !== 1) {
        throw new Error(
          `Expected one active marker, got ${activeCount}: ${JSON.stringify(markers)}`,
        );
      }
      if (markers.some((marker) => marker.text.trim() === "")) {
        throw new Error(
          `Blank marker after forward advance to DOM index ${domIndex}: ${JSON.stringify(markers)}`,
        );
      }
    }
    ...
    await page.waitForFunction(
      () => {
        ...
        return (
          markers.length === 9 &&
          markers.every((marker) => !marker.classList.contains("active"))
        );
      },
      { timeout: 5000 },
    );
